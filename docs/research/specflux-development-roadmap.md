...
- [ ] **Mouse movement affects terminal cursor** - When mouse hovers over embedded terminal, cursor moves on Claude Code's interactive menus. Claude Code enables mouse tracking mode which xterm.js reports to PTY. Filtering sequences at frontend/backend doesn't fully prevent this. **Workaround:** Use keyboard navigation. **Fix:** Investigate xterm.js CoreMouseService or report upstream.
